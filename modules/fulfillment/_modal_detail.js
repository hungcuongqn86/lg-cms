function ModalDetail() {
    this.printingbackendurl = '';
    this.prod_printing_id = null;
    this.getdatacmd = 'getProductPrintingInfo';
}

ModalDetail.prototype.init = function (module) {
    var loc = window.location;
    var pathName = loc.pathname.substring(0, loc.pathname.lastIndexOf('/') + 1);
    var absolutePath = loc.protocol + '//' + loc.host + pathName;
    this.printingbackendurl = absolutePath + 'modules/inc/detailModal.php';
    if ('printing' === module) {
        this.getdatacmd = 'getProductPrintingInfo';
    }
    if ('shipping' === module) {
        this.getdatacmd = 'getProductShippingInfo';
    }
    if ('print' === module) {
        this.getdatacmd = 'getCampaignOrderInfo';
    }
    if ('order' === module) {
        this.getdatacmd = 'getOrderInfo';
    }
};

ModalDetail.prototype.openModalPrinting = function (id, tab_select) {
    $('#modal-printing').modal('show');
    $('#modal-printing').each(reposition);
    $('.nav-tabs a[href="#' + tab_select + '"]').tab('show');
    this.prod_printing_id = id;
    this.getModalPrinting();
};

ModalDetail.prototype.getModalPrinting = function () {
    var myjs = this;
    ...
    $.ajax({
        url: myjs.printingbackendurl,
        type: "GET",
        dataType: 'json',
        data: {
            cmd: this.getdatacmd,
            id: myjs.prod_printing_id
        },
        success: function (data) {
            if (data && data.result) {
                ...
                if(myjs.getdatacmd !== 'getProductShippingInfo'){
...
//func save data
ModalDetail.prototype.updateState = function (state) {
    var myjs = this;
    $('#modal-printing-loader').show();
    $.ajax({
        url: myjs.printingbackendurl,
...
//func print data
ModalDetail.prototype._acceptData = function () {
    var myjs = this;
    $('#modal-printing-loader').show();
    $.ajax({
        url: myjs.printingbackendurl,
        ...
        dataType: 'json',
        data: {
            cmd: 'acceptProductPrintData',
            prod_print_arr: [myjs.prod_printing_id],
...
(function ($, AdminLTE) {
    $(document).ready(function () {
        $('#btn-accept-s').on('click', function (e) {
            modalDetail._acceptData();
        });

        $('#btn-deny-s').on('click', function (e) {
            modalDetail.updateState('deny');
        });

        $('#modal-btn-save-state').on('click', function (e) {
            modalDetail.updateState();
        });
    });
})(jQuery, $.AdminLTE);